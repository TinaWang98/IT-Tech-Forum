...
exports.login = (req,res) => {
    ...
    User.findOne({email}).exec((err,user) => {
        if(err || !user){
            return res.status(400).json({
                error:'User with that email does not exit. Please register.'
            })
        }
        //authenticate
...
exports.forgotPassword = (req,res) => {
    ...
    User.findOne({email}).exec((err,user) => {
        ...
        //populate the databse user to resetPasswordLink
        return user.updateOne({resetPasswordLink:token},(err,success) => {
            ...
            sendEmail
                .then(data => {
                    ...
                    return res.json({
                        message:`Email has been sent to ${email}. Click on the link to set your password`
                    })
                })
                .catch(error => {
                    console.log('ses reset password failed')
                    return res.json({
        ...
        jwt.verify(resetPasswordLink,process.env.JWT_RESET_PASSWORD,(err,success)=>{
            ...
            User.findOne({resetPasswordLink}).exec((err,user) => {
                ...
                user.save((err,result) => {
                    ...
                    res.json({
                        message:`Great! Now you can login with your password now`
                    })
                })
